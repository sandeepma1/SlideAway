    ...
    public class EasyObjectPool : MonoBehaviour
    {
        ...
        private void CheckForDuplicatePoolNames()
        {
            for (int index = 0; index < poolInfo.Length; index++)
            {
                ...
                for (int internalIndex = index + 1; internalIndex < poolInfo.Length; internalIndex++)
                {
                    ...
                    {
                        if (showDebug) Debug.LogError(string.Format("Pool {0} & {1} have the same name. Assign different names.", index, internalIndex));
                    }
                }
            }
        }
        ...
        private void CreatePools()
        {
            foreach (PoolInfo currentPoolInfo in poolInfo)
            {
                ...
                if (showDebug) Debug.Log("Creating pool: " + currentPoolInfo.poolName);
                //add to mapping dict
		...
		*/
        public GameObject GetObjectFromPool(string poolName, Vector3 position, Quaternion rotation)
        {
            ...
            if (poolDictionary.ContainsKey(poolName))
            {
                ...
                if (result == null)
                {
                    if (showDebug) Debug.LogWarning("No object available in pool. Consider setting fixedSize to false.: " + poolName);
                }

            }
            else
            {
                if (showDebug) Debug.LogError("Invalid pool name specified: " + poolName);
            }
        ...
        public void ReturnObjectToPool(GameObject go)
        {
            ...
            if (po == null)
            {
                if (showDebug) Debug.LogWarning("Specified object is not a pooled instance: " + go.name);
            }
            else
            {
                ...
                {
                    if (showDebug) Debug.LogWarning("No pool available with name: " + po.poolName);
                }
            }
        }
    }
